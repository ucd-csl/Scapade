...
from symspellpy_phonemes.editdistance import DistanceAlgorithm, EditDistance
...
class SymSpell(object):
    ...
    def __init__(self, max_dictionary_edit_distance=2, prefix_length=7,
        ...
        self._distance_algorithm = DistanceAlgorithm.LEVENSHTEIN_PHONEME
        self._max_length = 0
    ...
    def create_dictionary_entry(self, key, count):
        ...
        if len_key > self._max_length:
            self._max_length = len_key
            ...
            if candidate in self._deletes:
                ...
                for suggestion in dict_suggestions:
                    if suggestion == phrase:
                        continue
                    suggestion_len = len(suggestion.split())
    ...
    def _edits_prefix(self, key):
        len_key = len(key.split())
        hash_set = set()
        if len_key <= self._max_dictionary_edit_distance:
            hash_set.add("")
        if len_key > self._prefix_length:
            key = key.split()
